...
namespace Astrum
{
	...
	public:
		...
		virtual bool AddStateName(const std::string& name, const std::function<void(EStateType)>& callback) = 0;
		// 상태 정점을 여러개 추가합니다. 이미 생성된건 무시합니다.
		virtual void AddStateNames(const std::initializer_list<std::string>& names) = 0;
		virtual bool SetCallback(const std::string& name, const std::function<void(EStateType)>& callback) = 0;
		virtual std::function<void(EStateType)>& GetCallback(const std::string& name) = 0;
		...
		virtual bool InvokeCallback(const std::string& name, EStateType type) = 0;
