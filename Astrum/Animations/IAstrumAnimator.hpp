...
namespace Astrum {
	...
	enum AnimatingStatusType {
		// 애니메이션이 중단됨.
		AnimatingStatusType_Stop = -3,
		// 애니메이션이 취소됨.
		AnimatingStatusType_Undo = -2,
		// 애니메이션이 완료됨. 
		AnimatingStatusType_End = -1,
		// 애니메이션이 아직 시작되지 않음. (또는 시작시간이 되었더라도, 한번이라도 적용된 적이 아직 없음.)
		AnimatingStatusType_Wait = 0,
		// 애니메이션을 적용하는 중. (또는 종료시간이 되었더라도, 목표 값으로 아직 갱신되지 않음.)
		AnimatingStatusType_Animating = 1,
	};

	struct IAnimator {
		virtual ~IAnimator() = default;

		// 애니메이터를 준비합니다. IObject* owner가 할당된 상태여야만 합니다.
		...
		virtual void SetOwner(class IObject* obj) = 0;
		virtual class IObject* GetOwner() const = 0;
		virtual AnimatingStatusType GetStatus() const = 0;

		// 애니메이션의 시작 시간입니다. Chrono::GetRunningTime()을 기반으로 합니다. (기본값은 생성자가 호출된 시각입니다.)
