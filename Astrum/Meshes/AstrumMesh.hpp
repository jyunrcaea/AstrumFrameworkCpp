...
namespace Astrum
{
	...
	public:
		...
		Mesh(const MeshDescription<VertexType>& desc)
			: Mesh(desc.vertices, desc.indices, desc.primitive, desc.format, desc.vertexUsage, desc.indexUsage) {}
	...
	protected:
		...
		void UpdateVertexBuffer() {
			...
			auto* const ctx = Renderer::Instance().GetContext();
	...
	Mesh<VertexType>::Mesh(const std::vector<VertexType>& vertices,
		...
		if (false == Renderer::Instance().CreateBuffer(vertices, vertexBuffer, D3D11_BIND_VERTEX_BUFFER, vertexUsage)) {
	#if _DEBUG
		...
		// Create index buffer
		if (false == Renderer::Instance().CreateBuffer(indices, indexBuffer, D3D11_BIND_INDEX_BUFFER, indexUsage)) {
	#if _DEBUG
