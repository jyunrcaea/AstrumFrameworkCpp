...
namespace Astrum
{
	...
	RenderTarget::RenderTarget(unsigned int width, unsigned int height, DXGI_FORMAT format, unsigned int sampleCount)
		: width(width), height(height)
	{
		ID3D11Device* const device = Renderer::Instance().GetDevice();

		if (sampleCount >= 32) sampleCount = Renderer::Instance().GetSampleCount();
	...
	{
		ID3D11DeviceContext* const context = Renderer::Instance().GetContext();
	...
	void RenderTarget::Unbind()
	{
		...
		// 이전 상태로 복원
		Renderer::Instance().GetContext()->OMSetRenderTargets(1, previousRTV.GetAddressOf(), previousDSV.Get());
		previousRTV = nullptr;
		previousDSV = nullptr;
	}
	...
	void RenderTarget::ResetViewPort() const {
		D3D11_VIEWPORT vp{};
		vp.TopLeftX = 0.0f;
		vp.TopLeftY = 0.0f;
		...
		vp.MinDepth = 0.0f;
		vp.MaxDepth = 1.0f;
		Renderer::Instance().GetContext()->RSSetViewports(1, &vp);
	}

	void RenderTarget::ClearShaderResourceView() const {
		ID3D11ShaderResourceView* const nullSrv[1] = { nullptr };
		Renderer::Instance().GetContext()->PSSetShaderResources(0, 1, nullSrv);
	}
}