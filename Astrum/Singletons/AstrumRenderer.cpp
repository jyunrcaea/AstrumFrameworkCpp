...
namespace Astrum {
	void Renderer::Initialize(unsigned int width, unsigned int height, bool windowMode) {
	...
	#pragma endregion
		...
		mainRenderTarget = RenderTarget::MakeShared(width, height);

		TextureSampler::Instance().Initialize();
		TextureSampler::Instance().SetSampler(TextureSampleType_Linear);
	...
	void Renderer::Rendering() {
		RenderQueue::PeekToPreRender();

		const auto& c = Window::BackgroundColor();
	...
	void Renderer::Dispose() {
		...
		TextureSampler::Instance().Dispose(); // Same. Just for design.
		RenderQueue::Dispose();
	}

	ID3D11Device* Renderer::GetDevice() const { return device.Get(); }
	ID3D11DeviceContext* Renderer::GetContext() const { return context.Get(); }
	ID2D1RenderTarget* Renderer::GetRenderTarget2D() const { return renderTarget2D.Get(); }
	ID3D11DepthStencilView* Renderer::GetDepthStencilView() const { return depthStencilView.Get(); }

	void Renderer::CreateAndSetDefaultShapePipeline() {
		auto shapePipeline = ShaderSetup::MakeShared();
		shapePipeline->VertexShader = VertexShader::MakeShared(L"Astrum/Shaders/ColorMesh.fx", "ColorMeshVS");
		shapePipeline->PixelShader = PixelShader::MakeShared(L"Astrum/Shaders/ColorMesh.fx", "ColorMeshPS");
		...
		Renderer::Instance().DefaultShapeShaderPipeline = shapePipeline;
	}

	void Renderer::CreateAndSetDefaultMaterialPipeline() {
		auto texturePipeline = ShaderSetup::MakeShared();
		texturePipeline->VertexShader = VertexShader::MakeShared(L"Astrum/Shaders/Mesh.fx", "MeshVS");
		texturePipeline->PixelShader = PixelShader::MakeShared(L"Astrum/Shaders/Mesh.fx", "DefaultMaterialPS");
		...
		Renderer::Instance().DefaultTextureShaderPipeline = texturePipeline;
	}
	...
	Vector2 Renderer::GetRSRate() const {
		auto clientRect = Window::GetClientSize();
		return Vector2{
	...
	unsigned int Renderer::GetSampleCount() const { return sampleCount; }
}
