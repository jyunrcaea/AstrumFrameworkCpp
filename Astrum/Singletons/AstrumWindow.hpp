...
class AstrumWindow {
    AstrumWindow() = delete;
public:
    static inline bool Initialize(const std::wstring& title, unsigned int width, unsigned int height) { return AstrumWindowSingleton::Instance().Initialize(title, width, height); }
